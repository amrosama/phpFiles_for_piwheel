...
function getCourse($ID) {
    ...
    try {
        ...
        while ($row = mysqli_fetch_array($result)){
             $courseId = $row[1];
             if($row[2] == 1){
                $command = "select * from course where CourseID = '" . $courseId ."'";
                $res1= mysqli_query(connect(), $command) or die("Error:error on mysqli_query-> 2");
                while ($rowpur = mysqli_fetch_array($res1)){
             ...
             if($row[3] == 1){
                 $command = "select * from course where courseID = '" . $courseId ."'";
                $res2= mysqli_query(connect(), $command) or die("Error:error on mysqli_query-> 3");
                while ($rowcom = mysqli_fetch_array($res2)){
